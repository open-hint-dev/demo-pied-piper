...
**Emit (optional).** If this project registers an emitter, `hint emit <path...>` writes the artifact a spec produces and `hint emit --check` verifies that what is committed still matches. Only companion `<file>.hint` specs emit. Code outside the `hint:begin`/`hint:end` region is yours and is never touched; a `hint:hole(...)` region is where you write the implementation, and the constraints it must honor are listed directly above it. Once you fill a hole it is never overwritten — but if stderr says the spec changed since it was implemented, re-check that body against the spec.
